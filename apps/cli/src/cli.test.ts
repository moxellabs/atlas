...
`,
  ...
  test("mcp starts without requiring GitHub token config for public repos", async () => {
    const home = join(rootDir, "home-mcp-public");
    const mcpConfigPath = join(rootDir, "mcp-public.yaml");
    await writeFile(
      mcpConfigPath,
      `version: 1
...
`,
    ...
    const result = await runWithCapture(
      ["mcp", "--cwd", rootDir, "--config", mcpConfigPath],
      {
        HOME: home,
        GH_TOKEN: "",
        GITHUB_TOKEN: "",
...
`,
    ...
    const filteredSearch = await runWithCapture(
      ["search", "self-indexing", "--cwd", rootDir, "--audience", "consumer"],
      { HOME: home },
    );
