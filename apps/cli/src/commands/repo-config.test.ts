...
import { resolveRepoConfigInput } from "./repo-config";
...
describe("resolveRepoConfigInput", () => {
  test.each([
    ...
    "uses the %s origin as an editable local-git Repository ID fallback",
    async (_kind, remote) => {
      const root = await createGitCheckout(remote);
      const prompts = promptRecorder({
        "Repository ID": "github.com/example/edited",
      });

      const repo = await resolveRepoConfigInput(commandContext(root), input(), {
  ...
  test("prefers an explicit repository ID over the inferred origin", async () => {
    ...
    expect(prompts.inputs.map(({ question }) => question)).not.toContain(
      "Repository ID",
    );
  });
  ...
  test("requires an explicit repository ID in noninteractive local-git mode despite a parseable origin", async () => {
    ...
    await expect(
      resolveRepoConfigInput(commandContext(root), {
        ...input(),
        nonInteractive: true,
      }),
  ...
  test("keeps a blank defaultless Repository ID as CLI_REPO_ID_REQUIRED", async () => {
    ...
    await expect(
      resolveRepoConfigInput(commandContext(root), input(), {
        prompts: prompts.adapter,
      }),
    ).rejects.toMatchObject({ code: "CLI_REPO_ID_REQUIRED" });
    expect(prompts.inputs).toContainEqual({
  ...
  test("does not infer a local-git origin for GHES mode", async () => {
    ...
    expect(repo).toMatchObject({
      repoId: "github.example/platform/docs",
      mode: "ghes-api",
    });
    expect(prompts.inputs).toContainEqual({
      question: "Repository ID (host/owner/name, e.g. github.com/owner/repo)",
