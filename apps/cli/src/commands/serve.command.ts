...
import type { AtlasCliDependencies, CliCommandContext, CliCommandResult } from "../runtime/types";
...
export async function runServeCommand(context: CliCommandContext): Promise<CliCommandResult> {
...
export async function runServeCommandWithDependencies(
  ...
  try {
    ...
    const server = await deps.server.start({
      ...(host === undefined ? {} : { host }),
      ...(portValue === undefined ? {} : { port: Number.parseInt(portValue, 10) })
    });
    const url = `http://${server.host}:${server.port}`;
    const openRequested = context.argv.includes("--open");
    const openResult = openRequested ? await tryOpenUrl(url, openBrowser) : undefined;
    return renderSuccess(
      ...
      [
        ...
        ...(openResult === undefined
          ? []
          : [openResult.ok ? "Opened browser." : `Browser launch failed: ${openResult.error}`])
      ]
    );
  } finally {
    deps.close();
...
async function tryOpenUrl(url: string, openBrowser: (url: string) => Promise<void>): Promise<{ ok: boolean; error?: string | undefined }> {
  try {
