...
export function registerAtlasCommands(
  ...
  if (runtime.exposeIdentityOptions !== false) {
    const agent = configureCommandIo(new Command("agent"), runtime)
      .description("Manage Atlas MCP integrations for coding agents and IDEs")
      .usage("[options] <subcommand> [client]")
      .allowExcessArguments();
    for (const option of visibleOptions(globalOptions, runtime))
      agent.option(option.flags, option.description, option.parser as never);
    agent.action(async (...values: unknown[]) =>
      emitCommandResult(agent, runtime, values, runAgentCommand),
    );
    for (const spec of agentSubcommands)
      addSubcommand(
        agent,
        runtime,
        spec.name,
...
const agentServerOptions: OptionSpec[] = [
  {
...
const agentSubcommands = [
  ...
  },
  {
    ...
    args: ["[client]"],
    options: [...globalOptions, agentScopeOption, agentAllOption],
  },
  {
    name: "print-config",
