...
export function createAtlasCommand(config: AtlasMountConfig): Command {
  ...
  const displayName = config.displayName?.trim() || "knowledge";
  return createAtlasProgram(runtime, {
    name: namespace,
    commandName: namespace,
    identityName: displayName,
    description:
      "Local-first documentation ingestion, retrieval, and MCP/server access.",
    helpPrefix: `${namespace} <command>\nRuntime defaults: ${defaults.ATLAS_IDENTITY_ROOT ?? defaults.ATLAS_CACHE_DIR ?? "configured by host CLI"}\n`,
    helpQuickPath: mountedHelpQuickPath(namespace),
    exposeIdentityOptions: false,
    mountDefaults: defaults,
  });
}

function mountedHelpQuickPath(commandName: string): string {
  return `
Quick path:
  ...
  ${commandName} search <query>        search the local imported corpus
  ${commandName} mcp                   start MCP server over local corpus
`;
}
