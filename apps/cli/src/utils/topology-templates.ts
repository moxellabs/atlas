...
export type TopologyTemplate = "mixed-monorepo" | "package-top-level" | "module-local-docs";

/** Returns built-in topology rules for a supported template. */
export function topologyTemplate(template: TopologyTemplate): AtlasTopologyRule[] {
  ...
  return [
    {
      ...
      },
      {
        ...
        ownership: { attachTo: "module", moduleRootPattern: "{apps,packages}/*/src/**/docs/**/*.md" },
        authority: "preferred",
        priority: 30
      },
    {
...
/** Enumerates supported topology templates for prompts and help text. */
export function listTopologyTemplates(): Array<{ label: string; value: TopologyTemplate }> {
  return [
    { label: "Mixed monorepo", value: "mixed-monorepo" },
    { label: "Package top-level docs", value: "package-top-level" },
    { label: "Module-local docs", value: "module-local-docs" }
  ];
}
