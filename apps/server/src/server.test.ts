...
describe("server app", () => {
  ...
  test("fails closed and enforces authenticated read-only public access", async () => {
    ...
    const publicDependencies = createDependencies(store, dbPath, {
      enableMcp: true,
      host: "127.0.0.1",
      remote,
    });
  ...
  test("serializes concurrent MCP initialization before enforcing the session cap", async () => {
    const dependencies = createDependencies(store, dbPath, { enableMcp: true });
...
function remoteHeaders(token: string): HeadersInit {
  return {
