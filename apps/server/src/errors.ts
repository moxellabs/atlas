...
export class AtlasServerError extends Error {
  ...
  constructor(message: string, options: { code: string; status: number; context: ServerErrorContext }) {
    super(message);
...
/** Raised for intentionally reserved routes that require a future package. */
export class ServerUnsupportedOperationError extends AtlasServerError {
  constructor(message: string, context: ServerErrorContext) {
    super(message, { code: "unsupported_operation", status: 501, context });
  }
}
