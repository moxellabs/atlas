...
export function createContextRoutes(dependencies: AtlasServerDependencies) {
  return new Elysia({ name: "atlas-context-routes" }).post(
    "/api/context/plan",
    async ({ request }) => {
      const body = await parseJsonBody(request, planContextBodySchema, "planContext");
      return ok(requestIdFrom(request), presentContextPlan(dependencies.retrieval.planContext(compactPlanContext(body))));
    },
    docs.planContext
  );
}

function compactPlanContext(body: import("../schemas/context.schema").PlanContextBody) {
  return {
    query: body.query,
    budgetTokens: body.budgetTokens,
    ...(body.repoId === undefined ? {} : { repoId: body.repoId }),
    ...(body.candidateLimit === undefined ? {} : { candidateLimit: body.candidateLimit }),
    ...(body.summaryLimit === undefined ? {} : { summaryLimit: body.summaryLimit }),
    ...(body.expansionLimit === undefined ? {} : { expansionLimit: body.expansionLimit })
  };
}
