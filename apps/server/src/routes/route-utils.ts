...
async function readJsonBodyText(
  ...
  if (contentLength !== null) {
    ...
    if (Number.isFinite(declaredBytes) && declaredBytes > maxBytes) {
      throw new JsonBodyTooLargeError(operation, {
        maxBytes,
...
export async function parseJsonBody<T>(
  ...
  try {
    ...
    throw new ServerValidationError("Request body must be valid JSON.", {
      operation,
      entity: "body",
      cause: error,
    });
  ...
  try {
    ...
    throw new ServerValidationError("Request body validation failed.", {
      operation,
      entity: "body",
      cause: error,
      details: error,
...
export function parseQuery<T>(
  ...
  try {
    ...
    throw new ServerValidationError("Query parameters are invalid.", {
      operation,
      entity: "query",
      cause: error,
      details: error,
...
export function parseParams<T>(
  ...
  try {
    ...
    throw new ServerValidationError("Route parameters are invalid.", {
      operation,
      entity: "params",
      cause: error,
      details: error,
...
export function routeError(
  ...
  if (error instanceof RetrievalEntityNotFoundError) {
    set.status = 404;
    return fail(requestIdFrom(request), "not_found", error.message);
  }
  if (
