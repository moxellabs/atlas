...
Every release must pass `bun run audit:production` with zero known production dependency advisories. Any low, moderate, high, or critical advisory blocks release publication.

Atlas does not ship advisory suppressions by default. A future exception must be narrowly scoped to an exact advisory and documented here before an exact `--ignore` is added. The exception record must include:

- Advisory ID.
- Affected dependency path.
- Rationale and compensating controls.
- Named owner responsible for remediation.
- Expiry date after which the exception is removed or renewed explicitly.
