...
Every remote request requires the proxy's `x-forwarded-proto: https` assertion and the configured bearer token. The HTTP boundary exposes only health/version checks, the compact repository list, bounded search/context-planning endpoints, and MCP; repository details, document bodies/outlines, sync, build, config mutation, and other routes fail closed. Remote MCP advertises only bounded search, scope inference, and context-planning tools; aggregate resources and tools remain local-only. Repository IDs in API bodies and MCP tool arguments are checked against the allowlist. Requests use a bounded body size, a fixed-window per-token rate limit, and a bounded response size; oversized event streams are terminated.
