...
Atlas setup creates `~/.moxel/atlas/config.yaml`. Atlas config is loaded by `@atlas/config` from that user-home file when created through CLI setup/add-repo, from existing `atlas.config.yaml`, `atlas.config.yml`, or `atlas.config.json` project files, from an explicit `--config`, or from `ATLAS_CONFIG`. Enterprise Commander wrappers can mount Atlas with the same supported identity/default fields; see [Enterprise CLI Mount](enterprise-cli-mount.md).
...
`local-git` repos provide a Git remote, local cache path, and ref. `ghes-api` repos provide a GHES REST API base URL, owner, name, ref, and optional token env var.

`local-git` is for repositories that Atlas can clone or update in a local cache. `ghes-api` is for GitHub Enterprise Server repositories read through REST APIs. Credential discovery is resolved by `@atlas/config`; `@atlas/source-ghes` receives resolved auth metadata and performs API operations.
...
The CLI owns config mutation workflows such as `atlas setup` and repository registration. It creates `~/.moxel/atlas`, `~/.moxel/atlas/repos`, and the config/corpus parents as needed. Server-side mutation must preserve the same local-first behavior and should use config mutation helpers rather than editing YAML or JSON ad hoc.
...
Configured host web/API URLs are used to read `.moxel/atlas` artifact files during explicit `atlas add-repo`. Private repos should use least-privilege read tokens. Templates do not require additional write scopes.
...
Default identity root uses `.moxel/atlas` in a maintainer checkout and `~/.moxel/atlas` in a consumer runtime:
...
Custom identity root uses the public knobs `--atlas-identity-root`, `ATLAS_IDENTITY_ROOT`, and `identity.root`:

```bash
atlas setup --atlas-identity-root .acme/knowledge
ATLAS_IDENTITY_ROOT=.acme/knowledge atlas add-repo org/repo
```
...
Runtime state then derives under `~/.acme/knowledge`. Public docs and artifacts use document metadata fields `profile`, `audience`, `purpose`, and `visibility` to decide what belongs in a public artifact.
