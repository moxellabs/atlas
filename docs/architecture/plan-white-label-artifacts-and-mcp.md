...
MCP branding changes server metadata plus Atlas-prefixed resources and skill aliases. Generic tool names stay stable (`plan_context`, `search_passages`, `read_document`, `expand_related`, and `use_skill`) so clients can keep their routing rules while the displayed MCP server identity uses the configured brand.
