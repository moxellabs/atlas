...
export function treeToFileEntries(tree: GhesTreeResponse): FileEntry[] {
	if (tree.truncated) {
		throw new GhesTreeReadError({
			...
			message:
				"GHES tree response was truncated; refusing to return a partial file list.",
		});
	}
...
export function findTreeBlob(
	tree: GhesTreeResponse,
	path: string,
): GhesTreeItem | undefined {
	...
	return tree.tree.find(
		(item) =>
			item.type === "blob" && normalizeTreePath(item.path) === normalizedPath,
	);
}
