...
export function buildHitRationale(
	...
	if (factors.lexicalScore > 0) {
		rationale.push(
			`Search substrate contributed ${format(factors.lexicalScore)}.`,
		);
	}
	...
	if (factors.queryKind > 0) {
		rationale.push(
			`Query-kind policy contributed ${format(factors.queryKind)}.`,
		);
	}
	...
	if (factors.freshness > 0) {
		...
		rationale.push(
			`Candidate was penalized ${format(Math.abs(factors.freshness))} for stale repository freshness.`,
		);
	}
	if (factors.evidenceMatch > 0) {
		rationale.push(
