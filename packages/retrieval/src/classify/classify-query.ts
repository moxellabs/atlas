...
const SIGNALS: readonly SignalRule[] = [
  {
    ...
    weight: 4,
    patterns: [
      /\b(use|invoke|run|call|apply)\b.*\bskill\b/i,
      /\bskill\b.*\b(use|invoke|run|call|apply)\b/i,
    ],
  },
  {
    ...
    weight: 3,
    patterns: [
      /\b(error|bug|fail(?:ed|ing)?|fix|debug|troubleshoot|why does|not working|broken)\b/i,
    ],
  },
  {
    kind: "diff",
    ...
    weight: 3,
    patterns: [
      /\b(what changed|changes?|diff|updated?|modified|regression|since)\b/i,
    ],
  },
  {
    kind: "compare",
  ...
  },
  {
    ...
    weight: 3,
    patterns: [
      /\b(where is|where are|where do i find|which file|what file|location of|find (?:the )?(?:docs?|section|readme)|read (?:the )?(?:section|docs?))\b/i,
    ],
  },
  {
    kind: "usage",
    ...
    weight: 3,
    patterns: [
      /\b(how do i|how can i|how to use|how to|usage|example|examples|configure|configuration|setup|integrate|plan(?:ning)?|retrieve|retrieval|context|mcp|tools?|repo artifact|artifacts?|import|build|publish|init|sync|index|search|sqlite|fts|local|remote|upload|credentials?|tokens?|authentication|authorization)\b/i,
    ],
  },
  {
    kind: "overview",
    ...
    weight: 2,
    patterns: [
      /\b(what is|what are|overview|explain|how does|architecture|onboard|introduction)\b/i,
    ],
  },
  {
    kind: "exact-lookup",
    ...
    weight: 4,
    patterns: [
      /\b(exact|precise|specific|documented)\b.*\b(default|value|identity|knob|setting|name|rule|passage|behavior|policy|claim)\b/i,
  ...
  },
  {
    ...
    patterns: [
      /\b[\w.-]+\/[\w./-]+\b/i,
      /\b[\w.-]+\.(?:md|mdx|ts|tsx|js|jsx|json|yml|yaml)\b/i,
      /`[^`]+`/,
    ],
  },
] as const;
...
export function classifyQuery(query: string): QueryClassification {
  ...
  if (
    looksLikeShortIdentifier(trimmed) &&
    !scores.has("usage") &&
    !scores.has("overview")
  ) {
  ...
  if (matchedSignals.includes("path-or-symbol")) {
    if (scores.has("location")) {
      ...
      rationale.push(
        "Preferred explicit location intent over an embedded path signal.",
      );
    } else if (looksLikeNaturalLanguagePathMention(trimmed, scores)) {
      scores.set("exact-lookup", 1);
      rationale.push(
        "Softened path-like exact lookup signal inside a natural-language query.",
      );
    }
  }
...
function chooseKind(scores: ReadonlyMap<QueryKind, number>): QueryKind {
  return KIND_PRIORITY.reduce<QueryKind>((best, kind) => {
...
function confidenceFor(
  topScore: number,
  totalScore: number,
): QueryClassification["confidence"] {
