...
export interface SelectSummariesInput {
  ...
  /** Maximum summary items to select. Defaults by query kind and evidence need. */
  limit?: number | undefined;
}

/** Selects low-cost summaries and outlines before deeper evidence when policy allows it. */
export function selectSummaries(
  input: SelectSummariesInput,
): PlanningSelectionState {
  const limit =
    input.limit ??
    (input.queryKind === "exact-lookup"
      ? 0
      : needsConcreteEvidence(input.query)
        ? 1
        : input.queryKind === "overview" || input.queryKind === "compare"
          ? 2
          : 1);
  const state = cloneState(input.state);
  const summaryHits = input.rankedHits.filter(
    (hit) => hit.targetType === "summary",
  );
...
export function needsConcreteEvidence(query: string | undefined): boolean {
  ...
  return /`[^`]+`|\b[\w.-]+\/[\w./-]+\b|\b(?:bun|npm|pnpm|yarn|npx|node|git|atlas|mcp|sqlite|fts5?|import|build|publish|init|sync|upload|token|credentials?|tools?)\b/i.test(
    query,
  );
}

/** Appends a ranked hit if the current budget permits it; otherwise records it as omitted. */
export function appendIfBudgetAllows(
  state: PlanningSelectionState,
  hit: RankedHit,
  reason: string,
): boolean {
  ...
  if (item.tokenCount > state.budgetTokens) {
    state.omitted.push({
      ...item,
      rationale: [...item.rationale, "Item exceeds entire context budget."],
    });
    state.warnings.push(
      `Omitted ${item.targetType}:${item.targetId} because it exceeds the context budget.`,
    );
    return false;
  }
  if (state.usedTokens + item.tokenCount > state.budgetTokens) {
    state.omitted.push({
      ...
      rationale: [
        ...item.rationale,
        "Item does not fit remaining token budget.",
      ],
    });
    return false;
  }
  if (
    state.selected.some(
      (selected) =>
        selected.targetType === item.targetType &&
