...
import { RepoCacheService, requireLocalGitRepo } from "../cache/repo-cache.service";
...
export class LocalGitSourceAdapter implements RepoSourceAdapter {
  ...
  async readFile(repo: RepoConfig, path: string): Promise<SourceFile> {
    ...
    const filePath = resolveRepoPath(git.localPath, normalizedPath, repo.repoId);
    try {
 ...
 */
export async function listMaterializedFiles(rootPath: string): Promise<FileEntry[]> {
...
async function walkDirectory(rootPath: string, directoryPath: string, entries: FileEntry[]): Promise<void> {
...
function resolveRepoPath(rootPath: string, normalizedPath: string, repoId: string): string {
  if (normalizedPath.startsWith("../") || normalizedPath === ".." || normalizedPath.length === 0) {
    throw new GitReadFileError({
      repoId,
      localPath: rootPath,
      relativePath: normalizedPath
