...
import { buildDiffCommand, diffPaths } from "../diff/diff-paths";
import {
  GitCloneError,
...
test("terminates Git options before untrusted revision values", () => {
  expect(buildDiffCommand("--output=/tmp/owned", "main")).toEqual([
    "diff",
    "--name-status",
    "-z",
    "--find-renames",
