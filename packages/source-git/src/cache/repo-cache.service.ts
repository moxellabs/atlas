...
import { fetchUpdates, resolveCurrentRevision } from "./fetch-updates";
import type { FetchUpdatesResult } from "./fetch-updates";
import { checkoutCloneRef, ensurePartialClone, isGitRepoPath } from "./partial-clone";
import { applySparseCheckout } from "./sparse-checkout";
import type { SparseCheckoutResult } from "./sparse-checkout";
import { recordDiagnostic } from "../diagnostics";
import type { SourceGitDiagnosticEvent, SourceGitDiagnosticSink } from "../diagnostics";
import { GitInvalidRepositoryError, GitUnsupportedRepoModeError } from "../git/git-errors";
...
export interface RepoCacheServiceOptions {
  ...
  sparsePaths?: readonly string[] | ((repo: RepoConfig) => readonly string[]) | undefined;
...
export class RepoCacheService {
  ...
  async ensureCache(repo: RepoConfig): Promise<EnsureRepoCacheResult> {
    ...
    if (!existedBefore) {
      this.#record(diagnostics, event("clone_started", repo.repoId, git.localPath));
    }
    ...
    if (clone.cloned) {
      this.#record(diagnostics, event("clone_completed", repo.repoId, git.localPath));
    } else {
      this.#record(diagnostics, event("cache_validated", repo.repoId, git.localPath));
    }
    ...
    if (sparseCheckout.enabled) {
      ...
      this.#record(diagnostics, event("sparse_checkout_disabled", repo.repoId, git.localPath));
    }
  ...
  async updateCache(repo: RepoConfig): Promise<UpdateRepoCacheResult> {
    ...
    this.#record(diagnostics, event("fetch_started", repo.repoId, git.localPath));
    ...
    this.#record(
      diagnostics,
      event(update.changed ? "revision_changed" : "revision_unchanged", repo.repoId, git.localPath, {
  ...
  #resolveSparsePaths(repo: RepoConfig): readonly string[] {
    ...
    return typeof this.#options.sparsePaths === "function" ? this.#options.sparsePaths(repo) : this.#options.sparsePaths;
  }

  #record(diagnostics: RepoCacheDiagnosticEvent[], diagnostic: RepoCacheDiagnosticEvent): void {
 ...
 */
export function requireLocalGitRepo(repo: RepoConfig): NonNullable<RepoConfig["git"]> {
...
export function createRepoCacheDiagnostic(
  ...
  return details ? { type, repoId, localPath, details } : { type, repoId, localPath };
}
