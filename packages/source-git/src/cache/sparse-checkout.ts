 ...
 */
export async function applySparseCheckout(options: SparseCheckoutOptions): Promise<SparseCheckoutResult> {
  ...
  await runSparseCommand(options, ["sparse-checkout", "set", "--no-cone", ...patterns]);
...
async function runSparseCommand(
  ...
  if (result.exitCode !== 0) {
    if (allowUnsupportedDisable && isSparseCheckoutNotInitialized(result.stderr)) {
      return;
    }
...
function isSparseCheckoutNotInitialized(stderr: string): boolean {
  return stderr.includes("this worktree is not sparse") || stderr.includes("not in sparse checkout");
}
