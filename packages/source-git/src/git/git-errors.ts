...
export interface SourceGitErrorContext {
	...
	/** Git ref associated with ref-resolution failures. */
	ref?: string | undefined;
	/** Ref mode used for local-git resolution. */
...
export class SourceGitError extends Error {
	...
	constructor(
		code: SourceGitErrorCode,
		message: string,
		context: SourceGitErrorContext = {},
	) {
...
export class GitCloneError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		const message =
			context.refMode === "remote" && context.ref
				? `Git clone failed because ref ${context.ref} was not found on origin or could not be resolved. local-git remote mode fetches origin/${context.ref}; use refMode: current-checkout to build the current local checkout.`
				: "Git clone failed for the managed repo cache.";
		super("GIT_CLONE_FAILED", message, context);
	}
}
...
export class GitFetchError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_FETCH_FAILED",
			"Git fetch failed for the managed repo cache.",
			context,
		);
	}
}
...
export class GitInvalidRepositoryError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_INVALID_REPOSITORY",
			"Configured local path is not a valid Git repository.",
			context,
		);
	}
}
...
export class GitRefResolutionError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		const message =
			context.refMode === "remote" && context.ref
				? `Remote ref ${context.ref} was not found on origin or could not be resolved. local-git remote mode fetches origin/${context.ref}; use refMode: current-checkout to build the current local checkout.`
				: "Configured Git ref could not be resolved.";
		super("GIT_REF_RESOLUTION_FAILED", message, context);
	}
}
...
export class GitSparseCheckoutError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_SPARSE_CHECKOUT_FAILED",
			"Git sparse-checkout configuration failed.",
			context,
		);
	}
}
...
export class GitDiffError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_DIFF_FAILED",
			"Git diff failed while computing changed paths.",
			context,
		);
	}
}
...
export class GitReadFileError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_READ_FILE_FAILED",
			"Failed to read a file from the managed repo checkout.",
			context,
		);
	}
}
...
export class GitUnsupportedRepoModeError extends SourceGitError {
	constructor(context: SourceGitErrorContext = {}) {
		super(
			"GIT_UNSUPPORTED_REPO_MODE",
			'Repo config must use mode "local-git".',
			context,
		);
	}
}
