...
export function registerPlanContextTool(
	...
	return server.registerTool(
		PLAN_CONTEXT_TOOL,
		{
			title: "Plan local documentation context",
			description:
				options.description ?? "Find and package local evidence for a library, framework, API, or repository question. Returns coverage (sufficient, partial, absent, or stale), citations, and the next safe action. Use repoId when the source is known.",
...
export function registerSourcePlanContextTool(
	...
	return {
		...
		handle: server.registerTool(
			name,
			{
				title: `Plan local ${source.title} documentation`,
				description: `Plan local evidence for ${source.title}. Matches ${source.aliases.slice(0, 8).join(", ")}. Indexed topics include ${source.topics.slice(0, 24).join(", ")}. This source has ${source.documentCount} indexed documents and is currently ${source.fresh ? "fresh" : "stale"}.`,
				inputSchema: sourcePlanContextInputSchema,
				outputSchema: jsonOutputSchema,
				annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
			},
			(input) => toolResult(executePlanContext({ ...input, repoId: source.repoId }, dependencies)),
		),
	};
}

function uniqueCitations(selected: readonly { provenance: { repoId: string; path: string; docId: string } }[]) {
	const seen = new Set<string>();
	return selected.flatMap((entry) => {
		const citation = { repoId: entry.provenance.repoId, path: entry.provenance.path, docId: entry.provenance.docId };
