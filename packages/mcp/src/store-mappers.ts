...
export function listIndexedCoverage(
  db: StoreDatabase,
  limit?: number,
): IndexedCoverageItem[] {
  const count = (table: "packages" | "modules" | "documents", repoId: string) =>
    db.get<{ count: number }>(
      `SELECT COUNT(*) AS count FROM ${table} WHERE repo_id = $repoId`,
      { $repoId: repoId },
    )?.count ?? 0;
  const manifestRepository = new ManifestRepository(db);
  const repoRepository = new RepoRepository(db);
  const indexedRepoIds = db.all<{ repo_id: string }>(
    `SELECT repo_id
		 FROM manifests
		 ORDER BY repo_id
		 LIMIT $limit`,
  ...
  return indexedRepoIds.flatMap(({ repo_id: repoId }) => {
    const repo = repoRepository.get(repoId);
    if (repo === undefined) return [];
    const manifest = manifestRepository.get(repoId);
    ...
    return [
      {
        ...
        packageCount: count("packages", repo.repoId),
        moduleCount: count("modules", repo.repoId),
        documentCount: count("documents", repo.repoId),
      },
    ];
  });
}
