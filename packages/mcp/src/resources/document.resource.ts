...
import { getDocument, listSections, listSummaries } from "../store-mappers";
...
export const documentResource: AtlasResourceDefinition = {
  ...
  description:
    "Canonical document metadata, provenance, summaries, and outline.",
  read: (uri, dependencies) => {
    const docId = resourceId(uri);
    const document = getDocument(dependencies.db, docId);
    if (document === undefined) {
      throw new McpResourceNotFoundError("Document resource was not found.", {
        operation: "readDocumentResource",
        entity: docId,
      });
    }
    return {
