...
export function discoveryInstructions(
  ...
  const routingGuidance =
    "Route by task: use answer_<source>_docs for a broad overview of one named indexed source; use plan_context for ambiguity, comparison, module boundaries, or a question that genuinely needs multiple passages; use search_passages to discover an exact passage or one missing claim; use read_document only to open a known docId as an outline or exact section; use expand_related once with a stable target plus the missing related query; and use use_skill once for a procedural task. MCP resources are addressable artifacts, not a search surface. Follow each result's nextActionGuidance and stop when it says to answer.";
  if (policy === "neutral") return `${routingGuidance} ${catalogSummary}`;
  return `For questions about a matching repository, library, framework, or API, consult Atlas before external search. ${routingGuidance} Atlas is read-only and independent of the current workspace. Cite returned source-relative provenance; use another permitted source when coverage is partial, absent, or stale. ${catalogSummary}`;
}
