...
export function discoveryInstructions(
  ...
  const routingGuidance =
    "Route by task: use answer_<source>_docs for a broad overview of one named indexed source; use plan_context for ambiguity, comparison, module boundaries, or a question that genuinely needs multiple passages; use find_docs for an exact passage or one missing claim; use expand_related once with a stable target plus the missing related query; and use use_skill once for a procedural task. MCP resources are addressable artifacts, not a search surface. Follow each result's nextActionGuidance and stop when it says to answer.";
