...
function renderMcpAgent(effect: AgentEffectFreshness): string {
	...
	const metrics = effect.snapshot.metrics;
	return `<section class="tab-panel" id="mcp-agent" data-report-tab="mcp-agent" hidden>${tabHeader("Luna + Atlas MCP", "Actual MCP usage from the treatment arm; MCP use is observed, never forced.", freshnessNote(effect))}<div class="tab-kpi-grid four">${metricCard("Atlas adoption", percent(metrics.mcp.adoptionRate), "Treatment runs using local Atlas evidence", health(metrics.mcp.adoptionRate))}${metricCard("Atlas first", percent(metrics.mcp.atlasFirstRate), "Runs that chose local evidence before web", health(metrics.mcp.atlasFirstRate))}${metricCard("Local-only", percent(metrics.mcp.localOnlyRate), `Web fallback ${percent(metrics.mcp.fallbackRate)}`, health(metrics.mcp.localOnlyRate))}${metricCard("Protocol errors", metrics.mcp.protocolErrorRate.toFixed(2), "Average per traced treatment run", metrics.mcp.protocolErrorRate === 0 ? "good" : "bad")}</div><article class="data-card detail-card"><div class="card-heading"><div><span class="card-eyebrow">Representative traces</span><h2>Observed local-evidence interactions</h2></div></div>${representativePairs(effect.snapshot.pairs, effect.snapshot.representativeTraces).map(renderMcpTrace).join("") || "<p>No treatment trace used Atlas MCP in this snapshot.</p>"}</article></section>`;
}
