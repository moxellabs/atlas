...
function agentPrompt(task: AgentEffectTask): string {
  ...
  return `Evidence protocol (mandatory): Preserve the task's distinguishing names, paths, quoted terms, and constraints verbatim in evidence queries; do not replace them with a broad paraphrase. Do not add repository, scope, profile, audience, purpose, or visibility filters that the task did not request. Use the minimum evidence calls needed. A successful evidence result with nextAction=answer_locally, or guidance beginning TERMINAL that directs you to answer now, is a hard stop: return the required answer immediately. Any tool call after that result is an error. Use an external source only when indexed coverage explicitly says it is absent or stale, or when the task itself requires current external facts.\n\nTask:\n${task.prompt}\n\nReturn only the required JSON object. ${citationInstruction} Do not invent commands, files, or behavior; when evidence is unavailable, say so plainly.`;
}
