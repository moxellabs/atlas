...
function agentPrompt(task: AgentEffectTask): string {
  ...
  return `Task:\n${task.prompt}\n\nReturn only the required JSON object. ${citationInstruction} Preserve the task's distinguishing names, paths, quoted terms, and constraints verbatim in evidence queries; do not replace them with a broad paraphrase. Do not add repository, scope, profile, audience, purpose, or visibility filters that the task did not request. Use the minimum evidence calls needed. When an evidence tool marks a result terminal or directs you to answer now from its evidence, stop calling tools and answer from that packet. Use an external source only when indexed coverage explicitly says it is absent or stale, or when the task itself requires current external facts. Do not invent commands, files, or behavior; when evidence is unavailable, say so plainly.`;
}
