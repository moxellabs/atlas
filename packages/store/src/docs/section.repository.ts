...
export class SectionRepository {
  ...
  replaceForDocument(docId: string, sections: readonly CanonicalSection[]): SectionRecord[] {
    this.withRepositoryErrors("replaceSectionsForDocument", () => {
      this.db.transaction(() => {
        this.db.run("DELETE FROM chunks WHERE doc_id = $docId", { $docId: docId });
        this.db.run("DELETE FROM sections WHERE doc_id = $docId", { $docId: docId });
...
function insertSection(db: StoreDatabase, docId: string, section: CanonicalSection): void {
  db.run(
...
function mapSectionRow(row: SectionRow): SectionRecord {
  return {
    ...
    docId: row.doc_id,
    ordinal: row.ordinal,
    headingPath: decodeJsonArray<string>(row.heading_path_json, "sections.heading_path_json"),
    text: row.text,
    codeBlocks: decodeJsonArray(row.code_blocks_json, "sections.code_blocks_json")
  };
}
