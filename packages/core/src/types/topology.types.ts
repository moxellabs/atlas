import type { Authority, DiagnosticConfidence, DocKind } from "../enums";
import type { FileEntry, ModuleNode, PackageNode } from "./repo.types";

export type { Authority, DocKind };
...
export interface RepoTopologyAdapter {
  ...
  discoverModules(ctx: TopologyContext, packages: PackageNode[]): Promise<ModuleNode[]>;
  classifyDocs(ctx: TopologyContext, files: FileEntry[]): Promise<ClassifiedDoc[]>;
  classifySkills?(ctx: TopologyContext, files: FileEntry[]): Promise<SkillNode[]>;
}
