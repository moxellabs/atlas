(function () {
  const data = JSON.parse(
    document.getElementById("atlas-eval-report-data").textContent,
  );
  ...
  const previousPage =
    pagination && pagination.querySelector("button:first-of-type");
  const nextPage =
    pagination && pagination.querySelector("button:last-of-type");
  ...
  function apply(resetPage) {
    ...
    let visible = cards.filter(
      (c) =>
        ...
        (!profile.value || c.dataset.profile === profile.value) &&
        (!risk.value || c.dataset.risk === risk.value),
    );
    visible.sort((a, b) => {
      const key = sort.value;
      if (key === "id") return a.dataset.id.localeCompare(b.dataset.id);
      if (key === "latency")
        return Number(b.dataset.latency) - Number(a.dataset.latency);
      if (key === "ranked")
        return Number(b.dataset.ranked) - Number(a.dataset.ranked);
      if (key === "recallAt5")
        return Number(a.dataset.recall) - Number(b.dataset.recall);
      if (key === "mrr") return Number(a.dataset.mrr) - Number(b.dataset.mrr);
      return (
        Number(a.dataset.recall) - Number(b.dataset.recall) ||
        Number(a.dataset.mrr) - Number(b.dataset.mrr) ||
    ...
    if (pageLabel)
      pageLabel.textContent =
        (visible.length ? currentPage + 1 : 0) + " of " + totalPages;
    if (previousPage) previousPage.disabled = currentPage === 0;
    if (nextPage)
      nextPage.disabled = currentPage >= totalPages - 1 || visible.length === 0;
  }
  [search, cat, profile, risk, sort].forEach(
    (el) => el && el.addEventListener("input", () => apply(true)),
  );
  if (previousPage)
  ...
  function hidePopover() {
    if (popover) {
      popover.hidden = true;
      popover.innerHTML = "";
      popover.removeAttribute("data-open-for");
    }
  }
  function showPopover(btn) {
    ...
    popover.innerHTML =
      '<button type="button" class="info-close" aria-label="Close">×</button><h3>' +
      esc(entry.label) +
      "</h3><p>" +
      esc(entry.short) +
      '</p><p class="muted">' +
      ...
      esc(entry.interpretation) +
      '</p><p class="info-targets">Targets: ' +
      esc(entry.targets) +
      "</p>";
    ...
    if (left + popoverWidth > window.innerWidth - 12 + scrollX)
      left = window.innerWidth - popoverWidth - 12 + scrollX;
  ...
  function caseHealth(item) {
    if (!item.passed || item.retrieval.recallAt5 < 0.5) return "bad";
    if (
  ...
  function selectCase(id) {
    ...
    setText(
      "case-detail-expected",
      item.expectedBehavior ||
        "Required paths and terms present; forbidden paths absent; no-result behavior correct when expected.",
    );
    setText(
      "case-detail-claim",
      item.claim || "No answer-level claim is attached to this retrieval case.",
    );
    setText("case-detail-pass", item.passed ? "Passed" : "Failed");
    setText("case-detail-recall", pct(item.retrieval.recallAt5));
    setText(
      "case-detail-mrr",
      Number(item.retrieval.reciprocalRank).toFixed(2),
    );
    ...
    if (evidence)
      evidence.innerHTML = item.topPaths
        .slice(0, 6)
        .map(
          (path, index) =>
            "<tr><td>" +
        ...
        .join("");
    const pathCount = document.querySelector(
      ".case-evidence-section .card-heading > span:last-child",
    );
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (info) {
      ...
      if (
        popover &&
        popover.getAttribute("data-open-for") ===
          info.getAttribute("data-info-metric")
      ) {
        hidePopover();
        return;
      }
    ...
    if (popover && e.target.closest(".info-close")) {
      hidePopover();
      return;
    }
    if (
    ...
    if (detailTab) {
      document
        .querySelectorAll(".case-detail-tabs button")
        .forEach((button) =>
          button.setAttribute("aria-selected", String(button === detailTab)),
        );
      const targetByLabel = {
        Overview: "#case-detail-query",
        Retrieval: ".case-evidence-section",
        Diagnostics: "#case-detail-diagnosis",
        Metadata: ".case-metadata-grid",
      };
      const target = document.querySelector(
        targetByLabel[detailTab.textContent.trim()],
      );
      if (target)
    ...
    if (caseButton) {
      selectCase(caseButton.dataset.caseSelect);
      return;
    }
    const btn = e.target.closest("button");
    if (!btn) return;
    let text = btn.dataset.copyText;
    if (btn.dataset.copyId) {
      text = JSON.stringify(
        data.cases.find((c) => c.id === btn.dataset.copyId),
        null,
        2,
      );
    }
  ...
  function heatHtml(groups) {
    return Object.entries(groups)
      .map(([name, v]) => {
        const passH =
          v.passRate >= 1 ? "good" : v.passRate >= 0.95 ? "warn" : "bad";
        const rH =
          v.recallAt5 >= 0.8 ? "good" : v.recallAt5 >= 0.6 ? "warn" : "bad";
        const mH = v.mrr >= 0.6 ? "good" : v.mrr >= 0.35 ? "warn" : "bad";
        const order = { good: 0, warn: 1, bad: 2 };
        const worst = [passH, rH, mH].reduce(
          (a, b) => (order[b] > order[a] ? b : a),
          "good",
        );
        return (
          '<article class="heat" data-health="' +
          worst +
          '"><strong>' +
          esc(name) +
          "</strong><span>" +
          ...
          v.mrr.toFixed(2) +
          " · p95 " +
          Math.round(v.p95LatencyMs) +
          'ms</small><div class="pillrow">' +
          v.weakestCases
  ...
  if (group)
    group.addEventListener("change", () => {
      heat.innerHTML = heatHtml(data.quality[group.value]);
      title.textContent =
        "Quality by " + group.options[group.selectedIndex].text.toLowerCase();
    });
  ...
  function showTab(id, resetScroll) {
    const target =
      ...
      panels.find((panel) => panel.id === "overview");
    if (!target) return;
  ...
  navLinks.forEach((link) =>
    link.addEventListener("click", (event) => {
      ...
      if (location.hash === "#" + id) showTab(id, true);
      else location.hash = id;
    }),
  );
  window.addEventListener("hashchange", () =>
    showTab(decodeURIComponent(location.hash.slice(1)) || "overview", true),
  );
