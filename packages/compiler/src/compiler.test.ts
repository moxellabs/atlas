...
describe("compiler markdown pipeline", () => {
  test("preserves normalized content when frontmatter is absent", () => {
  ...
  test("extracts a valid frontmatter mapping and strips its delimiters", () => {
    expect(
      extractFrontmatter("---\ntitle: Hello\ntags:\n  - atlas\n---\n# Body"),
    ).toMatchObject({
      present: true,
  ...
  test("returns an empty mapping for an empty frontmatter block", () => {
  ...
  test("wraps malformed YAML in a path-bearing frontmatter error", () => {
    const parse = () =>
      extractFrontmatter("---\ntitle: [\n---\n# Body", {
        path: "docs/broken.md",
      });

    expect(parse).toThrow(CompilerFrontmatterError);
    expect(parse).toThrow(
      "Malformed frontmatter block. (stage=frontmatter, path=docs/broken.md)",
    );
  });

  test("rejects top-level frontmatter lists and scalars", () => {
    for (const yaml of ["- one", "value"]) {
      expect(() => extractFrontmatter(`---\n${yaml}\n---\nBody`)).toThrow(
  ...
  test("reports a missing frontmatter closing delimiter", () => {
    expect(() => extractFrontmatter("---\ntitle: Hello\n# Body")).toThrow(
      "Frontmatter opening marker is missing a closing marker.",
    );
  });

  test("normalizes CRLF frontmatter bodies and preserves trailing newlines", () => {
    expect(
      extractFrontmatter("---\r\ntitle: Hello\r\n---\r\n# Body\r\n"),
    ).toMatchObject({
      data: { title: "Hello" },
...
describe("document metadata classification", () => {
  ...
  test("applies built-in metadata defaults", () => {
    ...
    expect(
      compilePath("packages/core/README.md").document.metadata,
    ).toMatchObject({
      visibility: "public",
      audience: ["contributor"],
